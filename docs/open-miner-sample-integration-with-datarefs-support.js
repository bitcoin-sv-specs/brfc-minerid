...
// }
// export interface GetDataRefsOpReturnDto {
//  alias: string
// }
// export class MiningCandidateJob {
...
private async createNewJobAsync(template: MiningTemplate): Promise<MiningCandidateJob> {
  ...
  if (this.poolConfig.coinbase.minerId.enabled && this.providerConfig.minerIdConfig !== undefined) {
    ...
    // Request the Node to return its in-mempool dataRefs transaction id
    // (the getdatareftxid RPC interface is called).
    const getDataRefsTxId = async (): Promise<string> => {
       return await getDataRefTxId()
    }
    // Get datarefs op_return script(s) from the MID Generator
    // (the 'GET /datarefs/:alias/opreturns' method is called).
    const getDataRefsOpReturnScripts = async (): Promise<string[]> => {
       const dataRefsOpReturns: GetDataRefsOpReturnDto = {
         alias: this.providerConfig.minerIdConfig.alias
       }
	   return await this.minerId.getDataRefsOpReturnsAsync(dataRefsOpReturns)
    }
    // Get a miner-info op_return script from the MID Generator
    // (the 'GET /opreturn/:alias/:blockHeight([0-9]+)' or 'GET /opreturn/:alias/:blockHeight([0-9]+)/:dataRefsTxId' method is called).
    const getMinerInfoOpReturnScript = async (height, dataRefsTxId): Promise<string> => {
       const opReturn: GetOPReturnDto = {
           blockHeight: height,
           alias: this.providerConfig.minerIdConfig.alias
       }
       if (dataRefsTxId !== undefined) {
           opReturn.dataRefsTxId = dataRefsTxId
       }
    ...
    // Request the Node to create a datarefs transaction containing the specified datarefs script(s)
    // (the createdatareftx RPC interface is called).
    const createDataRefsTx = async (dataRefsOpReturnScripts) : Promise<string> => {
       if (!(Array.isArray(dataRefsOpReturnScripts) && dataRefsOpReturnScripts.length)) {
           return
       }
       return (await this.daemonManager.createDataRefTx({ scriptPubKeys: dataRefsOpReturnScripts }, {})).first().result
    }
    ...
    if (this.currentJob.height === miningCandidate.height) {
       ...
       if (!(await this.minerId.getIsOpReturnValidAsync(this.providerConfig.minerIdConfig.alias))) {
          // Request the Node to return its in-mempool datarefs transaction id.
          const dataRefsTxId = await getDataRefsTxId()
          // Get a miner-info op_return script from the MID Generator.
          const minerInfoOpReturnScript = await getMinerInfoOpReturnScript(miningCandidate.height, dataRefsTxId)
          // Request the Node to replace its miner-info transaction by a new one containing the specified miner-info script.
          minerInfoTxId = await replaceMinerInfoTx(minerInfoOpReturnScript)
          // Request the Node to return a new mining candidate with the miner-info dataRefs transactions included.
    ...
    {
       // Get datarefs op_return script(s) from the MID Generator.
	   const dataRefsOpReturnScripts = await getDataRefsOpReturnScripts()
       // Request the Node to create dataRefs transaction containing the specified script(s).
       const dataRefsTxId = await createDataRefsTx(dataRefsOpReturnScripts)
       // Get a miner-info op_return script from the MID Generator.
       const minerInfoOpReturnScript = await getMinerInfoOpReturnScript(miningCandidate.height, dataRefsTxId)
       // Request the Node to create a miner-info transaction containing the specified miner-info script.
       minerInfoTxId = await createMinerInfoTx(minerInfoOpReturnScript)
       // Request the Node to return a new mining candidate with the miner-info and dataRefs transactions included.
       template = (await this.getMiningTemplate(this.jobManager.currentJob.daemonId)).first()
    }
